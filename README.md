...
- ✅ **数据查询**：支持按时间范围查询SQLite数据库的宽表（data_wide）
  - 宽表查询：直接从 data_wide 表查询所有设备数据和命令数据
  - 聚合查询：从聚合库（{device_data.db}.agg.db 的 forecast_aggregate_wide_15m 表，15 分钟粒度）拉取并导出长时间历史数据（宽表保留期缩短后，长期数据保存在聚合库）
  - 流式导出：支持大数据量的流式导出，不占用内存
...
2. **配置查询参数**
   - 设置数据库路径（默认：`/mnt/analysis/data/device_data.db`）
   - 设置时间范围（可使用快捷按钮：今天/昨天/最近7天）
   - 程序会自动从 `data_wide` 表查询所有设备数据和命令数据

3. **执行查询或导出**
   - **查询模式**：点击"执行查询"按钮，查看查询结果（支持分页浏览）
   - **导出模式**：点击"导出CSV"按钮，选择保存位置，数据将流式导出为CSV格式
   - 导出功能支持大数据量，使用流式处理，不会占用大量内存

4. **查看导出结果**
   - CSV文件使用UTF-8 BOM编码，可直接用Excel打开
   - 时间戳自动格式化为东八区时间
   - `local_timestamp` 字段包含毫秒精度
